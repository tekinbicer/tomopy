...
BIN=${HOME}/projects/tomopy/test/test_trace/test_trace.py
INPUT=${WORK}/data/13id1_fixed_8s.h5
OUTPUT=${WORK}/data/r_13id1_fixed_8s.h5
ITER=20

ALGORITHM=trace_sirt
NPROCS=4
NTHREADS=64
python ${BIN} --input ${INPUT} --output ${OUTPUT} --iter ${ITER} --algorithm ${ALGORITHM} --ncores ${NPROCS} --nthreads ${NTHREADS}
NPROCS=4
NTHREADS=32
python ${BIN} --input ${INPUT} --output ${OUTPUT} --iter ${ITER} --algorithm ${ALGORITHM} --ncores ${NPROCS} --nthreads ${NTHREADS}
NPROCS=4
