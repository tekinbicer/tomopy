...
int DISPEngineBase<RST, DT>::NumProcessors(){
  ...
  if(num_procs<1)
    std::cout << "Unable to detect available number of processors." << std::endl;
  ...
  replication_type_(FULL_REPLICATION)
{
  ...
  num_reduction_threads_ = 
    (num_reduction_threads<1) ? NumProcessors() : num_reduction_threads;
  num_procs_ = num_reduction_threads;
