...
class TraceMetadata{
  private:
    ...
    float center_;
    float mov_;
    float *gridx_ = nullptr;
    float *gridy_ = nullptr;

    int const num_rays_proj_;
    int const num_rays_slice_;

    size_t const count_;
    ADataRegion<float> *recon_ = nullptr; 
    int const num_neighbor_recon_slices_;

    void SetupMovGrids(
        int const num_cols, int const num_grids, float const center,
        float &mov,
        float* &gridx, float* &gridy)
    {
      mov = static_cast<float>(num_cols)/2. - center;
      if(mov - std::ceil(mov) < 1e-6) mov += 1e-6;

      gridx = new float[num_grids+1];
      gridy = new float[num_grids+1];
      for(int i=0; i<=num_grids; i++){
        gridx[i] = -num_grids/2. + i;
        gridy[i] = -num_grids/2. + i;
      }
    }
  ...
  public:
    ...
    , num_neighbor_recon_slices_{num_neighbor_recon_slices}
    {
      ...
      // Setup grids and mov
      SetupMovGrids(num_cols_, num_grids_, center_, mov_, gridx_, gridy_);
    }
    ...
    , num_neighbor_recon_slices_{num_neighbor_recon_slices}
    {
      ...
      int num_recon_slices = num_slices+2*num_neighbor_recon_slices;
      recon_ = 
        new DataRegionBareBase<float>(num_recon_slices*num_grids_*num_grids_);
      if (recon_ == nullptr) throw std::invalid_argument("recon ptr is null");
